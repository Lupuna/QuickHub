...
from .test_base import SettingsPermissions


class TestCompanyAccessMixin(SettingsPermissions):
...
class TestProjectAccessMixin(SettingsPermissions):
...
class TestTaskAccessMixin(SettingsPermissions):
    ...
    def test_task_from_forbidden_project(self):
        url = reverse('team:task', args=[
            self.company.id,
            self.forbidden_project_user_company.id,
            self.task_from_forbidden_project_user_company.id
        ])
        response = self.auth_client.get(url)
        self.assertEqual(response.status_code, 403)
